    ...
    function (record, search, file, format, Papa, runtime, email, serverWidget) {
        ...
        function reduce(context) {
            try {
                ...
                for (var cA = 0; cA < contentArr.length; cA++) {
                    ...
                    log.debug("rSmartView", rSmartView);
                ...
                if (publisherName && rVbDate && rVbBillDueDate) {
                    //log.debug("vbQty", vbQty);
                    try {
            ...
            function vendorSearch(publisherName) {
                ...
                log.debug("vendorSearchResultCount", vendorSearchResultCount)
                ...
                //Start: search checks if there is any program name have Rev share BSS-768
                var customrecord_publishers_url_linkSearchObj = search.create({
                    type: "customrecord_publishers_url_link",
                    filters:
                        [
                            ["custrecord_publisher_name_vendor", "anyof", vendorId],
                            "AND",
                            ["custrecord_program_name", "anyof", "101"]
                        ],
                    columns:
                        [
                ...
                log.debug("customrecord_publishers_url_linkSearchObj result count", searchResultCount);
                if (searchResultCount <= 0) 
				{
                    return vendorId;
                }
            } // End: search checks if there is any program name have Rev share BSS-768
        ...
        function getBrandUrlData(vendor, rVbDate, siteIdArray, itemId) {
            ...
            customrecord_publishers_url_linkSearchObj.run().each(function (result) {
                ...
                log.debug("rVbDate", rVbDate); //2024-07-01
                fileDateCompWithURL = new Date(rVbDate.split("-")[0], rVbDate.split("-")[1] - 1, rVbDate.split("-")[2]); // formatFileDate(rVbDate);
